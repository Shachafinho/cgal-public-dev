...
public:
  ...
  typedef typename Arrangement_on_surface_2::Observer
                                                Base;
...
public:
  ...
  Arr_trapezoid_ric_point_location (const Arrangement_on_surface_2& arr,
                           ...
                           double depth_thrs = CGAL_TD_DEFAULT_DEPTH_THRESHOLD,
                           double size_thrs = CGAL_TD_DEFAULT_SIZE_THRESHOLD) :
    Base(const_cast<Arrangement_on_surface_2 &>(arr)),
...
protected:
  ...
  void _construct_td ()
  {
    ...
    for (eit = this->arrangement()->edges_begin(); eit != this->arrangement()->edges_end(); ++eit)
    {
      he_cst = eit;
